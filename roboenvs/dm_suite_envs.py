...
#   env = DeepMindWrapper(env, (84, 84), camera_id=camera_id)
#   env = ActionRepeat(env, action_repeat)
#   if normalize:
#     env = NormalizeActions(env)
#   env = MaximumDuration(env, max_length)
#   env = PixelObservations(env, (84, 84), np.uint8, 'image')
#   env = ConvertTo32Bit(env)
#   env = ConcatObservation(env, ['image'])
#   return env
#
#
...
#         DeepMindWrapper.__init__(self,ee,(64,64),camera_id=0)
#     def reset(self):
#         return DeepMindWrapper.reset(self)
#     def step(self,action):
#         transition = DeepMindWrapper.step(self,action)
...
#
# import roboenvs.nested_dmwrap as nested_dmwrap
#
#
...
#   def __init__(self, env):
#     self._env = env
#     self._outdir = os.path.join(os.getcwd(),'curious_planet')
#     self._episode = None
#
#   def __getattr__(self, name):
#     return getattr(self._env, name)
...
#   def step(self, action):
#     observ, reward, done, info = self._env.step(action)
#     observ = nested_dmwrap.map(self._convert_observ, observ)
#     reward = self._convert_reward(reward)
#     #observ, reward, done, info = self._process_step(action,observ,reward,done,info)
#     return observ, reward, done, info
#
#   def reset(self):
#     observ = self._env.reset()
#     observ = nested_dmwrap.map(self._convert_observ, observ)
#     #observ = self._process_reset(observ)
#     return observ
#
